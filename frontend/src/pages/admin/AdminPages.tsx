...
export const SubsidyAllocation: React.FC = () => {
  ...
  const reviewMutation = useMutation({
    mutationFn: ({ id, action, ...rest }: any) => subsidyAPI.review(id, { action, ...rest }),
    onSuccess: (_,v) => { toast.success(`Application ${v.action}d`); qc.invalidateQueries({queryKey:['subsidy-applications-admin']}); qc.invalidateQueries({queryKey:['admin-dashboard']}); },
    onError: (e:any) => toast.error(e.response?.data?.message||'Failed'),
  });

  const disburseMutation = useMutation({
    mutationFn: ({ id, ...d }: any) => subsidyAPI.disburse(id, d),

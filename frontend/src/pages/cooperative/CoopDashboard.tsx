...
import { analyticsAPI, inputsAPI } from '../../api/client';
...
export default function CoopDashboard() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Distribution approved'); qc.invalidateQueries({queryKey:['coop-dashboard']}); },
  ...
  if (!d) return <div className="card text-center py-16 text-gray-400"><p>No cooperative found for your account. Contact admin.</p></div>;

  const { cooperative, stats, farmers, pending_requests, stock } = d;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-4"><h3 className="font-semibold text-gray-900">Pending Requests</h3><span className="bg-orange-500 text-white text-xs w-6 h-6 rounded-full flex items-center justify-center font-bold">{pending_requests?.length||0}</span></div>
          {pending_requests?.length===0?<p className="text-gray-400 text-sm text-center py-8">No pending requests</p>
          :pending_requests?.map((r:any)=>(
            <div key={r.id} className="bg-orange-50 border border-orange-100 rounded-xl p-3 mb-3">
              ...
              <div className="flex gap-2">
                ...
                <button className="flex-1 border border-gray-300 text-gray-600 text-xs py-1.5 rounded-lg flex items-center justify-center gap-1 hover:bg-gray-50"><XCircle size={11}/>Reject</button>
              </div>
            </div>
          ))}
        </div>

...
import { useAdminDashboard, usePendingUsers, useUserApproval } from '../../hooks';
import { QueryErrorBanner } from '../../components/common';

const AdminDashboard: React.FC = () => {
  const { stats, activities, isLoading, isError, refetch } = useAdminDashboard();
  const { users: pendingList, isLoading: pendingLoading } = usePendingUsers(5);
  const { updateStatus } = useUserApproval();

  const handleApprove = (userId: string) => updateStatus.mutate({ id: userId, status: 'active' });
  const handleReject = (userId: string) => updateStatus.mutate({ id: userId, status: 'rejected' });
              ...
              {pendingList.map((u: any) => (
                <div key={u.id} className="bg-orange-50 border border-orange-100 rounded-xl p-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => handleApprove(u.id)} disabled={updateStatus.isPending} className="flex-1 bg-agri-green text-white text-xs py-1.5 rounded-lg flex items-center justify-center gap-1 hover:bg-agri-lightgreen">
                      <CheckCircle size={11} /> Approve
                    </button>
                    <button onClick={() => handleReject(u.id)} disabled={updateStatus.isPending} className="flex-1 border border-gray-300 text-gray-600 text-xs py-1.5 rounded-lg flex items-center justify-center gap-1 hover:bg-gray-50">

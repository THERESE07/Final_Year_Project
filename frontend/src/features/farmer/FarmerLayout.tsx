...
import { LayoutDashboard, Package, DollarSign, Shield, Bell, User, LogOut, Menu, Search, ClipboardList } from 'lucide-react';
import { useAuth } from '../../contexts/AuthContext';
import NotificationBell from '../../components/common/NotificationBell';
import toast from 'react-hot-toast';

const navItems = [
  { path: '/farmer', label: 'My Dashboard', icon: LayoutDashboard },
  { path: '/farmer/inputs', label: 'My Inputs', icon: Package },
  { path: '/farmer/input-requests', label: 'Input Requests', icon: ClipboardList },
  { path: '/farmer/subsidies', label: 'My Subsidies', icon: DollarSign },
  { path: '/farmer/security', label: 'Security & Sessions', icon: Shield },
  { path: '/farmer/notifications', label: 'Notifications', icon: Bell },
  { path: '/farmer/profile', label: 'My Profile', icon: User },
];
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-52 bg-white border-r border-gray-100 flex flex-col z-10">
            <SidebarContent navItems={navItems} location={location} user={user} onLogout={handleLogout} onClose={() => setSidebarOpen(false)} />
          </aside>
        </div>
      )}
      <aside className="hidden lg:flex flex-col w-52 bg-white border-r border-gray-100 flex-shrink-0">
        <SidebarContent navItems={navItems} location={location} user={user} onLogout={handleLogout} />
      </aside>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3 flex-shrink-0">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <NotificationBell linkTo="/farmer/notifications" />
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                <User size={16} className="text-agri-green" />
              </div>
              <div className="hidden sm:block">
                <p className="text-xs font-medium text-gray-800">Farmer</p>
                <p className="text-xs text-gray-400">ID: {user?.id?.slice(0, 7).toUpperCase() || ''}</p>
              </div>
            </div>
...
const SidebarContent: React.FC<{ navItems: any[]; location: any; user: any; onLogout: () => void; onClose?: () => void }> = ({ navItems, location, onLogout, onClose }) => (
  <div className="flex flex-col h-full">
    ...
    <div className="p-4 border-b border-gray-100">
      <div className="flex items-center gap-2 mb-1">
        ...
        <div>
          <p className="text-xs font-bold text-gray-900 leading-tight">AgriSubsidy System</p>
    ...
    <div className="px-4 py-2 mt-2">
      <p className="text-xs text-gray-400 font-medium">Navigation Menu</p>
      <p className="text-xs font-semibold text-agri-green">Farmer Portal</p>
    </div>
    ...
    <nav className="flex-1 px-2 space-y-0.5 overflow-y-auto">
      {navItems.map(item => {
        ...
        const active = location.pathname === item.path || (item.path !== '/farmer' && location.pathname.startsWith(item.path));
        return (

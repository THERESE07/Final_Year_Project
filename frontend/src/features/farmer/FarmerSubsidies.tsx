...
import { useFarmerSubsidies } from '../../hooks';
import { QueryErrorBanner } from '../../components/common';
import { parsePayload, subsidyApplicationPayloadSchema } from '../../utils/schemas';
...
export default function FarmerSubsidies() {
  ...
  const { applications: apps, programs: progs, isLoading, isError, refetch } = useFarmerSubsidies();

  const applyMutation = useMutation({
    mutationFn: (d: Record<string, unknown>) => subsidyAPI.apply(d),
    onSuccess: () => {
      toast.success('Application submitted');
      qc.invalidateQueries({ queryKey: [QUERY_KEYS.FARMER_SUBSIDIES] });
      qc.invalidateQueries({ queryKey: [QUERY_KEYS.FARMER_DASHBOARD] });
      setShowApply(false);
      setSelectedProgram('');
      setAmount('');
      setReason('');
  ...
  const handleApply = () => {
    const parsed = parsePayload(subsidyApplicationPayloadSchema, {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="space-y-4">{Array(3).fill(0).map((_,i)=><div key={i} className="card h-32 animate-pulse bg-gray-100"/>)}</div>
      ) : (
            ...
            <div key={a.id} className="card">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900">{a.program?.name||a.program_name||'—'}</h3>
                    <p className="text-xs text-gray-400 capitalize">{a.program?.type||a.program_type} subsidy</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3"><button onClick={()=>setShowApply(false)} className="flex-1 btn-secondary">Cancel</button><button onClick={handleApply} disabled={!selectedProgram||!amount||!reason||applyMutation.isPending} className="flex-1 btn-primary">{applyMutation.isPending?'Submitting...':'Submit Application'}</button></div>
            </div>
          </div>
        </div>
      )}
